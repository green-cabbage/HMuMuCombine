...
# ROOT.gStyle.SetNdivisions(1010, "XYZ")
...
def read(scan, param, files, ycut):
    ...
    graph = plot.TGraphFromTree(limit, param, "2*(deltaNLL)", "quantileExpected > -1.5") # nll0 is just one value, so plot1Dscan rejects it 
    graph.SetName(scan)
    graph.Sort()
    print(f"graph.GetN(): {graph.GetN()}")
...
parser.add_argument("--y-cut", type=float, default=20000007.0, help="Remove points with y > y-cut")
parser.add_argument("--y-max", type=float, default=20000008.0, help="y-axis maximum")
...
# args.breakdown = None # we're not doing any breakdown here
print("--------------------------------------")
print(args.output)
print("--------------------------------------")
...
canv = ROOT.TCanvas(args.output, args.output, 1000, 800)
# canv = ROOT.TCanvas(args.output, args.output)
pads = plot.OnePad()
main_scan["graph"].SetMarkerColor(1)
main_scan["graph"].GetYaxis().SetMaxDigits(2)
main_scan["graph"].SetTitle("Curve of 2*(deltaNLL+nll+nll0)")

# main_scan["graph"].Draw("AP")
main_scan["graph"].Draw("AC")
# main_scan["graph"].GetYaxis().SetDecimals(2)
...
# h.GetYaxis().SetTitle("- 2 #Delta ln L")
...
# axishist.GetYaxis().SetDecimals(2)
axishist.GetYaxis().SetMaxDigits(2)
...
textfit = "" # we're not doing any textfit, and they give wrong numbers
