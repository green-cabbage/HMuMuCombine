...
#     # print()
#     rows = tree.GetSelectedRows()
#     getattr(tree, "deltaNLL")
#     tree.Print()
#     # rows.Print()
#     print(rows)
#     # tree.GetV1().Print()
...
def extractNLL(t):
    ...
    # NLL_vals = np.array(NLL_vals, dtype=np.float64)
    # NLL_vals = NLL_vals/1e6
    r_vals = array("d", r_vals)
    NLL_vals = array("d", NLL_vals)
    print(f"r_vals : {r_vals}")
    print(f"NLL_vals : {NLL_vals}")
    print(f"r_vals : {len(r_vals)}")
    print(f"NLL_vals : {len(NLL_vals)}")

    return (r_vals), (NLL_vals)


def TGraphFromTree(tree, xvar, yvar, selection):
    x_vals, y_vals = extractNLL(tree)
    n_points = len(x_vals)
    gr = R.TGraph(n_points, x_vals, y_vals)
    print(f"gr.GetN() : {gr.GetN()}")
    return gr
